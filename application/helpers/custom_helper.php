...
//is cache active
if (!function_exists('is_cache_active')) {
    function is_cache_active($cached_active)
    {
        if (@$cached_active == 1) {
            return true;
        } else {
...
//get post hit count
if (!function_exists('get_post_hit_count')) {
    function get_post_hit_count($post_id)
    {
        // Get a reference to the controller object
        $ci =& get_instance();
        return $ci->post_model->get_post_hit_count($post_id);
    }
}
