...
class Post_controller extends Admin_Core_Controller
{
    ...
    */
    ...
    public function update_post($id)
    {
        ...
        } elseif ($data['post']->post_type == "gallery") {
            $this->load->view('admin/post/update_gallery', $data);
        } elseif ($data['post']->post_type == "ordered_list") {
            $this->load->view('admin/post/update_ordered_list', $data);
        } else {
    ...
    */
    ...
    {
        check_permission('add_post');
        $data['title'] = trans("add_gallery");
        $data['top_categories'] = $this->category_model->get_top_categories();

        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/post/add_gallery', $data);
        $this->load->view('admin/includes/_footer');
    }
     ...
     * Gallery Post Items
     */
    public function gallery_post_items($id)
    {
        check_permission('add_post');
        $data['title'] = trans("gallery_post_items");
        $data['post'] = $this->post_admin_model->get_post($id);
        if (empty($data['post'])) {
            redirect($this->agent->referrer());
        }
        $data['post_items'] = $this->post_item_model->get_gallery_post_items($id);

        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/post/post_items_gallery', $data);
        $this->load->view('admin/includes/_footer');
    }
    ...
    public function add_gallery_post()
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($this->post_admin_model->add_post($post_type)) {
                ...
                $this->post_item_model->add_gallery_post_item($last_id, 1);

                redirect(admin_url() . "gallery-post-items/" . $last_id);
            } else {
                $this->session->set_flashdata('form_data', $this->post_admin_model->input_values());
                $this->session->set_flashdata('error', trans("msg_error"));
                redirect($this->agent->referrer());
    ...
    /**
     * Update Gallery Post Item Post
     */
    public function update_gallery_post_item_post()
    {
        check_permission('add_post');
        if ($this->post_item_model->update_gallery_post_item()) {
            $item_id = $this->input->post('id', true);
            $this->session->set_flashdata('success', trans("msg_updated"));
            $this->session->set_flashdata('msg_post_item' . $item_id, 1);
            redirect($this->agent->referrer() . "#item" . $item_id);
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
        }
        redirect($this->agent->referrer());
    ...
    /**
     * Add Gallery Post Item Post
     */
    public function add_gallery_post_item_post()
    {
        check_permission('add_post');
        $post_id = $this->input->post('post_id', true);
        $item_order = $this->input->post('item_order', true);
        if ($this->post_item_model->add_gallery_post_item($post_id, $item_order)) {
    ...
    /**
     * Set Gallery Item Collapsed
     */
    public function set_gallery_item_box_collapsed()
    {
        $id = $this->input->post('id', true);
        $this->post_item_model->set_gallery_item_box_collapsed($id);
    }

    /**
     * Delete Gallery Post Item Post
     */
    public function delete_gallery_post_item_post()
    {
        check_permission('add_post');
        $item_id = $this->input->post('item_id', true);
        $this->post_item_model->delete_gallery_post_item($item_id);
    }
    ...
    */
     ...
     * Add Ordered List
     */
    public function add_ordered_list()
    {
        check_permission('add_post');
        $data['title'] = trans("add_ordered_list");
        $data['top_categories'] = $this->category_model->get_top_categories();

        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/post/add_ordered_list', $data);
        $this->load->view('admin/includes/_footer');
    }

    /**
     * Add Ordered List Post
     */
    public function add_ordered_list_post()
    {
        check_permission('add_post');
        //validate inputs
        $this->form_validation->set_rules('title', trans("title"), 'required|xss_clean|max_length[500]');
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($this->post_admin_model->add_post($post_type)) {
                ...
                //insert post tags
                $this->tag_model->add_post_tags($last_id);
                //add ordered list post item
                $this->post_item_model->add_ordered_list_item($last_id, 1);

                redirect(admin_url() . "ordered-list-items/" . $last_id);
            } else {
                $this->session->set_flashdata('form_data', $this->post_admin_model->input_values());
                $this->session->set_flashdata('error', trans("msg_error"));
                redirect($this->agent->referrer());
     ...
     * Ordered List Items
     */
    public function ordered_list_items($id)
    {
        check_permission('add_post');
        $data['title'] = trans("ordered_list_items");
        $data['post'] = $this->post_admin_model->get_post($id);
        if (empty($data['post'])) {
            redirect($this->agent->referrer());
        }
        $data['post_items'] = $this->post_item_model->get_ordered_list_items($id);

        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/post/post_items_ordered_list', $data);
        $this->load->view('admin/includes/_footer');
    }
     ...
     */
    public function update_ordered_list_item_post()
    {
        check_permission('add_post');
        if ($this->post_item_model->update_ordered_list_item()) {
            $item_id = $this->input->post('id', true);
            $this->session->set_flashdata('success', trans("msg_updated"));
            $this->session->set_flashdata('msg_post_item' . $item_id, 1);
            redirect($this->agent->referrer() . "#item" . $item_id);
    ...
    /**
     * Add Ordered List Item Post
     */
    public function add_ordered_list_item_post()
    {
        check_permission('add_post');
        $post_id = $this->input->post('post_id', true);
        $item_order = $this->input->post('item_order', true);
        if ($this->post_item_model->add_ordered_list_item($post_id, $item_order)) {
            redirect($this->agent->referrer() . "#footer");
        }
        redirect($this->agent->referrer());
    }
     ...
     */
    public function set_ordered_list_item_box_collapsed()
    {
        $id = $this->input->post('id', true);
        $this->post_item_model->set_ordered_list_box_collapsed($id);
    }

    /**
     * Delete Ordered List Item Post
     */
    public function delete_ordered_list_item_post()
    {
        check_permission('add_post');
        $item_id = $this->input->post('item_id', true);
        $this->post_item_model->delete_ordered_list_item($item_id);
    }
    ...
    */
    ...
    /**
     * Update Post Items Options Post
     */
    public function update_post_items_options_post()
    {
        $post_id = $this->input->post('post_id', true);
        if ($this->post_item_model->update_post_items_options($post_id)) {
            $this->session->set_flashdata('success', trans("msg_updated"));
            $this->session->set_flashdata('msg_post_options', 1);
            reset_cache_data_on_change();
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
        }
