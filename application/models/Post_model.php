...
class Post_model extends CI_Model
{
    ...
    //get last posts
    public function get_last_posts($lang_id, $limit)
    {
        $this->db->join('users', 'posts.user_id = users.id');
        $this->db->join('categories', 'posts.category_id = categories.id');
        $this->db->select('posts.*, categories.name as category_name, categories.color as category_color, users.username as username, users.slug as user_slug');
    ...
    {
        $sql = "SELECT posts.id, posts.title, posts.title_slug, posts.post_type, posts.image_small, posts.image_mime, posts.image_url, users.slug AS user_slug, users.username AS username, posts.created_at, hit_counts.count AS hit FROM posts 
                ...
                WHERE posts.is_scheduled = 0 AND posts.status = 1 AND posts.lang_id = ? ORDER BY hit_counts.count DESC LIMIT 5";
    ...
    //get rss posts by user
    public function get_rss_posts_by_user($user_id)
    {
        $this->set_filter_query();
    ...
    //get post hit count
    public function get_post_hit_count($post_id)
    {
        $this->db->where('post_id', $post_id);
        $query = $this->db->get('post_hits');
        return $query->num_rows();
    }
